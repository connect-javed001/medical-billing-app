     ...
     public function loadMedList(){
             ...
             if(!empty($items)){
                   foreach ($items as $key => $value) {
					   $text = $value['Medicine']['name'] .' '.$value['Medicine']['batch_no'];
                        $data[] = array('id' => $value['Medicine']['id'], 'text' => $text);			 	
                   } 
                } else {
...
//            debug($data);exit;
    ...
    public function plusStock($id,$qty){
		...
		$checkZero = $this->Medicine->findById($id);
		if(empty($checkZero['Medicine']['quantity']) || $checkZero['Medicine']['quantity']<1){
